...
class SegTrainer2(BaseTrainer):
    ...
    def build_model(self):
        if self.model_name != 'Unknown' and self.model is None:
            ...
            if args.dataset == 'btcv':
                ...
                self.loss_fn = FocalLossCustom(alpha=0.69857891, gamma=1.0)
            else:
    ...
    def run(self):
        ...
        best_metric = 0
        best_ts_metric = 0
        #wandb.log({'fold_id':args.fold_id})
    ...
    @staticmethod
    def batch_generator(tuple1, tuple2, batch_size):
        """Generate batches from a tuple"""
        for i in range(0, len(tuple1), batch_size):
            yield tuple1[i:i + batch_size], tuple2[i:i + batch_size]
    ...
    def evaluate(self, epoch=0, niters=0, get_only_output=False):
        ...
        with torch.no_grad():
            for i, batch_data in enumerate(val_loader):
                ...
                for metric_name, metric_func in self.metric_funcs.items():
                    ...
                    compute_avg_metric(metric.cpu().numpy(), log_meters, metric_name, batch_size, args)
                ...
                if args.dataset == 'prostate':
                    ...
                    all_valid_preds.append(perm_pred.detach().cpu())
                    all_valid_labels.append(perm_target.detach().cpu())
        ...
        if args.dataset == 'prostate':
            ...
            try:
                ...
                batch_for_metrics = 16
                for bdet, btarget in self.batch_generator(final_det,final_target, batch_for_metrics):
                    # Calculate num_parallel_calls safely
                    #num_cores = os.cpu_count() or 1  # Handle None case
                    #num_parallel_calls = max(1, num_cores - 2)  # Ensure at least 1
                    ...
                    # valid_metrics = evaluate(
                    #             y_det=iter(final_det),
                    #             y_true=iter(final_target),
                    #             y_det_postprocess_func=lambda pred: extract_lesion_candidates(pred)[0],
                    #             num_parallel_calls = 1,
                    #             verbose=1
                    #             )
                    # valid_metrics = custom_evaluate(
                    #     y_det=iter(final_det),
                    #     y_true=iter(final_target),
                    #     verbose=1  # Shows progress
                    # )
                    auroc_list.append(valid_metrics.auroc)
                    ap_list.append(valid_metrics.AP)
                #import pdb;pdb.set_trace()
        ...
        if args.rank == 0 and not args.disable_wandb:
            ...
            if args.dataset == 'prostate':
                wandb_log_dict['AUROC'] = auroc #valid_metrics.auroc
                wandb_log_dict['AP'] = ap #valid_metrics.AP
